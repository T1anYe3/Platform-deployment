...
echo "[restore] === Platform 1 Restore from: ${BACKUP_PATH} ==="
...
sleep 2
curl -s ${CURL_AUTH} -X POST "${ES_URL}/_snapshot/platform1-backup/${SNAPSHOT_NAME}/_restore" \
  -H "Content-Type: application/json" \
  -d '{"indices": "safeline-records-*,suricata-alerts-*,vault-audit-*,minio-audit-*,nifi-logs-*"}' >/dev/null 2>&1 || echo "  (restore may have partially failed - check ES logs)"
...
if [ -d "${BACKUP_PATH}/vault-data" ]; then
  ...
  docker cp "${BACKUP_PATH}/vault-data/." platform1-vault:/vault/data/ 2>/dev/null || echo "  (vault data copy skipped)"
