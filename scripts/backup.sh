...
echo "[backup] === Platform 1 Backup: ${TIMESTAMP} ==="
echo "[backup] Output: ${BACKUP_PATH}"

# 1. Register ES snapshot repository in MinIO
echo "[backup] Registering ES snapshot repository in MinIO..."
curl -s ${CURL_AUTH} -X PUT "${ES_URL}/_snapshot/platform1-backup" \
...
curl -s ${CURL_AUTH} -X PUT "${ES_URL}/_snapshot/platform1-backup/${SNAPSHOT_NAME}?wait_for_completion=true" \
  -H "Content-Type: application/json" \
...
docker cp platform1-vault:/vault/data "${BACKUP_PATH}/vault-data" 2>/dev/null || echo "  (vault data copy skipped)"

# 4. Volume metadata
echo "[backup] Saving volume metadata..."
docker volume ls --filter name=platform1 --format json > "${BACKUP_PATH}/volumes.json" 2>/dev/null || true
