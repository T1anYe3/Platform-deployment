#!/usr/bin/env python3
"""Bridge: NiFi system diagnostics -> Elasticsearch (Platform 2)"""
...
def index_events(events):
    ...
    for doc in events:
        ...
        lines.append(json.dumps({'index': {'_index': f'platform2-nifi-logs-{date}', '_id': uid}}))
        lines.append(json.dumps(doc))
    es_bulk_index(lines)
    return len(events)

def main():
    ensure_index_template('platform2-nifi-logs', NIFI_MAPPINGS)
