#!/bin/sh
# Initialize Vault: unseal, enable engines, store Platform 2 secrets
...
echo "[vault-init] Storing Platform 2 secrets..."

# MinIO credentials
vault kv put -tls-skip-verify secret/platform2/minio \
  access_key="${MINIO_ROOT_USER:-minioadmin}" \
  secret_key="${MINIO_ROOT_PASSWORD:-ChangeThis-Local-123!}" \
  endpoint="https://minio:9000" 2>/dev/null || echo "  [skip] secret/platform2/minio (may already exist)"

# NiFi admin credentials
vault kv put -tls-skip-verify secret/platform2/nifi \
  admin_user="${NIFI_ADMIN_USER:-admin}" \
  admin_pass="${NIFI_ADMIN_PASS:-Admin123!ChangeMe}" \
  endpoint="https://nifi:8443" 2>/dev/null || echo "  [skip] secret/platform2/nifi (may already exist)"

# Platform 2 config paths
vault kv put -tls-skip-verify secret/platform2/config \
  es_url="http://elasticsearch:9200" \
  kibana_url="http://kibana:5601" \
  minio_buckets="raw-data,processed-data,model-files,evaluation-results,archive-data,audit-evidence" 2>/dev/null || echo "  [skip] secret/platform2/config (may already exist)"
