#!/bin/sh
# Platform 3 Functional Health Check
# Verifies core services and ES indices are accessible and functional
...
if [ "$OUTPUT_JSON" != "true" ]; then
  echo ""
  echo "========================================"
  echo "  Platform 3 Functional Health Check"
  echo "========================================"
  echo ""
fi

# Core services (using container-internal addresses)
...
# Check ES indices
echo ""
[ "$OUTPUT_JSON" != "true" ] && echo "  ES Index Status:"

for idx in vault-audit minio-audit nifi-logs safeline-records suricata-alerts; do
  count_result=$(curl -s "http://elasticsearch:9200/${idx}-*/_count" 2>/dev/null | grep -o '"count":[0-9]*' | head -1 || echo "N/A")
  [ "$OUTPUT_JSON" != "true" ] && echo "    ${idx}-*: ${count_result}"
done
...
if [ "$OUTPUT_JSON" = "true" ]; then
  ...
  echo "    Vault:  https://localhost:18200"
  echo ""
fi
